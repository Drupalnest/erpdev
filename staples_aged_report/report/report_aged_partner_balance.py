...
class ReportAgedPartnerBalance(models.AbstractModel):
    ...
    _name = 'report.staples_aged_report.report_agedpartnerbalance'
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length):
        ...
        query = '''
            SELECT DISTINCT l.partner_id, res_partner.name AS name, UPPER(res_partner.name) AS UPNAME, res_partner.ref as ref, COALESCE(res_partner.phone, res_partner.mobile) as phone, CASE WHEN prop.value_text IS NULL THEN 'normal' ELSE prop.value_text END AS trust
        ...
        # This dictionary will store the ytd sales of all partners
        query = '''SELECT l.partner_id, sum(l.balance) as amount
                    FROM account_move_line AS l, account_account, account_move am
                    WHERE 	(l.account_id = account_account.id) 
                        AND (l.move_id = am.id)
                        AND (am.state IN %s)
        ...
        cr.execute(query, (tuple(move_state), tuple(account_type), tuple(partner_ids), (date_from - relativedelta(years=1)), date_from, tuple(company_ids)))
        ytd_sales = {x[0]: x[1] for x in cr.fetchall()}
        ...
        for partner in partners:
            ...
            for i in range(4):
                ...
                if not float_is_zero(values[str(i)], precision_rounding=self.env.company.currency_id.rounding):
                    at_least_one_amount = True
            values['total'] = sum([values['direction']] + [values[str(i)] for i in range(4)])
            # Add for total
            ...
            if partner['partner_id']:
                ...
                values['ytd_sales'] = partner['partner_id'] in ytd_sales and ytd_sales[partner['partner_id']] or 0.00
            else:
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if not data['form'].get('date_from', False):
        ...
        if data['form']['partner_ids']:
            ...
            self = self.with_context(partner_ids=partner_ids)
